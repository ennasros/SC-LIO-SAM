...
#include "camera_models/OusterCamera.h"
...
public:
    ...
    n(n_in)
    {
        ...
        pointsArray.resize(64);
        for (int i = 0; i < (int)pointsArray.size(); ++i)
            pointsArray[i].resize(1024);
    }

    sensor_msgs::ChannelFloat32 os_get_depth(const std_msgs::Header& header, const cv::Mat& imageCur, 
                                                const pcl::PointCloud<PointType>::Ptr& laserCur,
                                                const camodocal::CameraPtr& camera_model ,
                                                const vector<geometry_msgs::Point32>& features_2d)
    {
        ros::Time stamp_cur = header.stamp;

        ROS_DEBUG("num points: %lu, image pixels: %d", laserCur->size(), imageCur.rows*imageCur.cols);

        // initialize depth for return
        sensor_msgs::ChannelFloat32 depth_of_point;
        depth_of_point.name = "depth";
        depth_of_point.values.resize(features_2d.size(), -1);

        // make sure synced depth is available
        if (laserCur->size() == 0)
            return depth_of_point;

       // project normalized 2d features onto a unit sphere
        pcl::PointCloud<PointType>::Ptr features_3d(new pcl::PointCloud<PointType>());
        for (int i = 0; i < (int)features_2d.size(); ++i)
        {
            ...
            features_3d->push_back(p);
        }

        // project laser scan cloud on range image
        cv::Mat rangeImage = cv::Mat(imageCur.rows, imageCur.cols, CV_32F, cv::Scalar::all(FLT_MAX));
        for (int i = 0; i < (int)laserCur->size(); ++i)
        {
            Eigen::Vector2d p_2d;

            PointType p = laserCur->points[i];
            // compute angles
            float row_angle = atan2(-p.z, sqrt(p.x * p.x + p.y * p.y)) * 180.0 / M_PI + 22.5; // degrees, up -> down, 0 -> 360
            float col_angle = atan2(p.y, p.x) * 180.0 / M_PI + 180.0; // degrees, left -> right, 0 -> 360
            // find id in range image
            int row_id = round(row_angle/45*64);
            int col_id = round(col_angle/360*1024);
            // discard point if id is out of bounds
            if (row_id < 0 || row_id >= 64 || col_id < 0 || col_id >= 1024)
                continue;
        ...
        // publish used laser scan
        publishCloud(&pub_depth_cloud, laserCur, stamp_cur, "os_sensor");

        // project depth cloud onto a unit sphere
        ...
        {
            ROS_WARN("Unit sphere has less that 10 depth cloud points..");
            return depth_of_point;
        }

        // create a kd-tree using the spherical depth cloud
        pcl::KdTreeFLANN<PointType>::Ptr kdtree(new pcl::KdTreeFLANN<PointType>());
        kdtree->setInputCloud(depth_cloud_unit_sphere);

        // find the feature depth using kd-tree
        vector<int> pointSearchInd;
        vector<float> pointSearchSqDis;
        float dist_sq_threshold = pow(sin(3.0 / 180.0 * M_PI) * 5.0, 2);
        for (int i = 0; i < (int)features_3d->size(); ++i)
        {
            kdtree->nearestKSearch(features_3d->points[i], 3, pointSearchInd, pointSearchSqDis);
            if (pointSearchInd.size() == 3 && pointSearchSqDis[2] < dist_sq_threshold)
            {
                ...
                Eigen::Vector3f V(features_3d->points[i].x,
                                  features_3d->points[i].y,
                                  features_3d->points[i].z);
                ...
                // if (max_depth - min_depth > 15 || s <= 0)
                // {
                //     // ROS_WARN("depth and s req not satisfied. diff = %f, s = %f", max_depth-min_depth, s);
                //     continue;
                // } else
                 if (s - max_depth > 0) {
                    s = max_depth;
                ...
                features_3d->points[i].x *= s;
                features_3d->points[i].y *= s;
                features_3d->points[i].z *= s;

                // the obtained depth here is for unit sphere, VINS estimator needs depth for normalized feature (by value z), (lidar x = camera z)
                features_3d->points[i].intensity = features_3d->points[i].z;
                
                std::stringstream ss;
                ss << "max_depth - min_depth = " << (max_depth - min_depth)
                   << "\ns  = " << s
                   << "\nA' = " << A.transpose()
                   << "\nB' = " << B.transpose()
                   << "\nC' = " << C.transpose()
                   << "\nfinal used s  = " << s
                   << "\nfinal feature point = " << features_3d->points[i];
                ROS_DEBUG("%s", ss.str().c_str());
            }
            else
            {
                // ROS_WARN("couldn't find matching depth");
            }
        }
        // visualize features in cartesian 3d space (including the feature without depth (default 1))
        publishCloud(&pub_depth_feature, features_3d, stamp_cur, "os_sensor");

        // update depth value for return
        for (int i = 0; i < (int)features_3d->size(); ++i)
        {
            if (features_3d->points[i].intensity > 0)
                depth_of_point.values[i] = features_3d->points[i].intensity;
        }
        ...
        if (pub_depth_image.getNumSubscribers() != 0)
        {
            ...
            for (int i = 0; i < (int)laserCur->size(); ++i)
            {
                ...
                Eigen::Vector3d p_3d(laserCur->points[i].x,
                                     laserCur->points[i].y,
                                     laserCur->points[i].z);
                ...
                // p_2d << col_angle/360*1024, row_angle/45.0*64.0;
                camera_model->spaceToPlane(p_3d, p_2d);

                // std::stringstream ss;
                // ss << "space to plane conversion: " 
                //    << "\n3d  = " << p_3d
                //    << "\n2d' = " << p_2d;
                // ROS_DEBUG("%s", ss.str().c_str());

                points_2d.push_back(cv::Point2f(p_2d(0), p_2d(1)));
                points_distance.push_back(pointDistance(laserCur->points[i]));
            }
                                          ...
                                          const vector<geometry_msgs::Point32>& features_2d)
    {
        ...
        try{
            listener.waitForTransform("odom", "os_sensor", stamp_cur, ros::Duration(0.03));
            listener.lookupTransform("odom", "os_sensor", stamp_cur, transform);
        } 
        catch (tf::TransformException ex){
            ROS_ERROR("Error while getting transform from `os_sensor` to `odom` (feature_tracker.h)");
            return depth_of_point;
        }
        ...
        float bin_res = 360.0 / (float)num_bins; // currently only cover the space in front of lidar (-90 ~ 90)
        ...
        for (int i = 0; i < (int)depth_cloud_local->size(); ++i)
        {
            ...
            // filter points that are too close to the lidar
            if (sqrt(p.x * p.x + p.y * p.y) < 0.2)
                continue;
            if (abs(atan2(p.z, sqrt(p.x * p.x + p.y * p.y)) * 180.0 / M_PI ) > 22.5)
                continue;
        ...
        *depth_cloud_local = *depth_cloud_local_filter2;
        publishCloud(&pub_depth_cloud, depth_cloud_local, stamp_cur, "os_sensor");
        ...
        for (int i = 0; i < (int)features_3d_sphere->size(); ++i)
        {
            ...
            if (pointSearchInd.size() == 3 && pointSearchSqDis[2] < dist_sq_threshold)
            {
                ...
                ss << "max_depth - min_depth = " << (max_depth - min_depth)
                   ...
                   << "\nC' = " << C.transpose()
                   << "\nfinal used s  = " << s
                   << "\nfinal feature point = " << features_3d_sphere->points[i];
                ROS_DEBUG("%s", ss.str().c_str());
            }
        }
        ...
        {
            std::stringstream ss;
            ss << "features_3d_sphere point " << i << ":\t" << features_3d_sphere->points[i];
            ROS_DEBUG("%s",ss.str().c_str());
        }
        // visualize features in cartesian 3d space (including the feature without depth (default 1))
        publishCloud(&pub_depth_feature, features_3d_sphere, stamp_cur, "os_sensor");
